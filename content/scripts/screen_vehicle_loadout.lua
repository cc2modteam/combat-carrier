...
function get_selected_vehicle_attachment_options(attachment_type, attachment_index)
    ...
    if g_launched then
        ...
        if attachment_index > 13 and attachment_index < 18 then
            -- laser VLS
            add_attachment_option(attachment_options, e_game_object_type.attachment_hardpoint_missile_laser)
        end
...
function _update(screen_w, screen_h, ticks)
    ...
    if g_launched == false then
        -- select the carrier in drydock if we arent launched
        attached_vehicle = this_vehicle
        this_vehicle = find_drydock()
            ...
            if attached_vehicle:get() then
                ...
                if is_show_attachment_selector then
                    ...
                    if attachment:get() then
                        ...
                        local selection_options = get_selected_vehicle_attachment_options(attachment_type, g_selected_attachment_index)
...
function add_attachment_option(options, definition_index)
...
function render_screen_attachment(screen_w, screen_h, this_vehicle, attached_vehicle, is_active)   
    ...
    if attached_vehicle:get() then
        if g_selected_attachment_index ~= -1 then
            ...
            print(string.format("options for index=%d type=%d", g_selected_attachment_index, attachment_type))
            ...
            ui:begin_window(update_get_loc(e_loc.attachment).."##attachment", 0, 95, screen_w, screen_h - 95, nil, is_active, 1)
                ...
                if is_pressed then  
                    ...
                    if g_selected_option_index == 0 then
                        ...
                        g_screen_index = 1
                    elseif inventory_item_type == -1 or
                            ((not g_launched) or this_vehicle:get_inventory_count_by_definition_index(definition_index) > 0)
                    then
